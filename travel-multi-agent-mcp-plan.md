...
目标是新增一个“出行企划多智能体”系统：用户问“明天去哪玩”，系统自动理解需求、查询地点/天气/路线/预算，生成行程方案，最后复用现有 `EmailAgent` 给朋友发送邀约邮件。这里暂时不使用 Skills，优先朝 MCP 适配方向设计。
...
```text
Router + Orchestrator + Subagents + MCP Adapter + Handoff to EmailAgent
```

不要用一个大 Agent 做完所有事。出行企划天然包含多个外部能力：地图、天气、路线、POI、邮件。更好的做法是：

```text
Agent 负责判断、拆解、合并、审核
MCP 负责连接外部服务能力
EmailAgent 负责邮件草稿和发送确认
```
...
Travel Router
  判断任务类型：travel_plan_with_email

Travel Orchestrator
  补齐城市、日期、偏好、预算、朋友、邮箱等槽位

Subagents 并行调研
  DestinationAgent  查 POI / 景点 / 餐饮 / 活动
  WeatherAgent      查天气 / 温度 / 降雨 / 风险
  TransportAgent    查路线 / 通勤时间 / 交通方式
  BudgetAgent       估算门票 / 餐饮 / 通勤成本

PlannerAgent
  汇总成一份可执行的一日游计划

ReviewAgent
  检查时间冲突、天气风险、路线过长、预算超限、缺少邮箱

Handoff to EmailAgent
  生成邀约邮件草稿
  用户确认后发送
```
...
```text
travel_plan_only          只规划行程
travel_plan_with_email    规划行程并约朋友
travel_modify             修改已有行程
email_only                只发邮件
weather_only              只查天气
transport_only            只查路线
```
...
这些 Agent 之间不要互相乱调。统一由 Orchestrator 调度，结果统一进 Planner。
...
```text
TravelReviewAgent -> EmailAgent
```

Travel 系统只把结构化行程和邮件意图交给 EmailAgent。EmailAgent 继续负责：
...
```json
{
  ...
  "confidence": 0.92,
  "required_next_action": "collect_slots"
}
```
