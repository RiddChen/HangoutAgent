...
技术实现上不要只靠一个大 Agent 自由发挥，也不要第一版就手写完整的 raw `StateGraph`。推荐先沿用当前 `EmailAgent` 的模式：使用 LangChain v1.x 的 `create_agent`，由更高层 API 隐藏 LangGraph 的节点细节。

```text
LangChain create_agent
  负责每个 Agent 的 LLM 调用、工具调用、结构化输出、middleware、interrupt

LangGraph runtime
  仍然作为 create_agent 底层运行时，负责 checkpoint、Command、interrupt、streaming 等能力

Python Orchestrator
  第一版负责业务流程调度：先日期、再天气、再出发地、再路线、再计划、再可选邮件

MCP
  负责地图、天气、POI、路线等外部能力接入
```

也就是说，本项目实际落地应该是：

```text
LangChain create_agent 实现 RouterAgent / DateAgent / WeatherAgent / RouteAgent 等 Subagent
Python Orchestrator 控制调用顺序和条件分支
AsyncSqliteSaver / Command / interrupt 等仍然来自底层 LangGraph
langchain-mcp-adapters 接入高德地图 MCP 等外部工具
EmailAgent 作为用户明确邀约时的可选 handoff agent
复杂到需要可视化节点和精细条件边时，再在 create_agent 之上增加 raw StateGraph
```
...
weather_node
        |
        v
weather_decision_edge
        |                         |
        | good_weather             | bad_weather
        v                         v
ask_origin_node           recommend_date_node
        |                         |
        v                         v
route_node                wait_user_date_confirm
        |
        v
route_decision_edge
        |                         |
        | accepted                 | rejected
        v                         v
planner_node              preference_node -> route_node
        |
        v
review_node
        |
        v
save_latest_plan_node
        |
        v
END
...
第一版这里不要求手写 raw `StateGraph`。`app/agents/travel/orchestrator.py` 可以先是一个普通 Python 调度器，内部按状态调用 `DateAgent`、`WeatherAgent`、`RouteAgent` 等 `create_agent` 实例。因为 `create_agent` 底层已经使用 LangGraph，所以仍然可以复用 checkpoint、`Command`、interrupt、streaming 等机制。等流程复杂到需要显式节点、条件边、子图和可视化调试时，再把 Orchestrator 升级成 raw `StateGraph`。
